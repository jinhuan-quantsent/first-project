...
export async function login(email: string, password: string): Promise<AuthResponse> {
  const { data } = await client.post<ApiResponse<AuthResponse>>('/api/v1/auth/login', { email, password });
  return data.data;
}
...
export async function register(email: string, password: string): Promise<{ user_id: string; email: string; email_confirmed: boolean; message: string }> {
  const { data } = await client.post<ApiResponse>('/api/v1/auth/register', { email, password });
