...
async def _run_sector_price_update() -> None:
    """
    每日板块价格更新：使用 AKShare 拉取31个申万一级行业指数日线数据，
    写入 sector_price 表。在市场快照后执行（15:35）。
    """
    if not await _is_trade_day():
        logger.info("[Scheduler] 今日无交易数据，跳过板块价格更新")
        return
    ...
    today = date.today()
    logger.info("[Scheduler] 开始每日板块价格更新 -- %s", today.isoformat())
    ...
    async with AsyncSession(engine) as session:
        for sector_code, sector_name in SW_L1_SECTORS_DAILY:
            try:
                ...
                df["pct_change"] = df["pct_change"].round(4).fillna(0)
                ...
                sql = text("""INSERT INTO sector_price
                    (sector_code, sector_name, trade_date, open, high, low, close, volume, amount, pct_change)
                    VALUES (:sector_code, :sector_name, :trade_date, :open, :high, :low, :close, :volume, :amount, :pct_change)
                    ON DUPLICATE KEY UPDATE
                    sector_name = VALUES(sector_name), open = VALUES(open), high = VALUES(high),
                ...
                fail += 1
                logger.warning("[Scheduler] board price %s failed: %s", sector_code, e)

        await session.commit()

    logger.info("[Scheduler] board price update done -- success %d, fail %d", success, fail)


# ============================================================
# 任务 12：融资融券数据更新（AKShare 上交所融资融券）
# ============================================================

async def _run_market_margin_update() -> None:
    """
    每日融资融券数据更新：使用 AKShare stock_margin_sse() 拉取上交所融资融券数据，
    写入 market_margin 表。在因子更新后执行（16:15）。
    """
    if not await _is_trade_day():
        logger.info("[Scheduler] 今日无交易数据，跳过融资融券更新")
        return
    ...
    today = date.today()
    logger.info("[Scheduler] 开始每日融资融券更新 -- %s", today.isoformat())

    engine = get_async_engine()

    try:
        df = ak.stock_margin_sse(date=today.strftime("%Y%m%d"))
        if df is None or df.empty:
            logger.warning("[Scheduler] margin data empty, may not be published yet")
            return

        async with AsyncSession(engine) as session:
            for _, row in df.iterrows():
                # AKShare stock_margin_sse columns vary, handle multiple formats
                ...
                margin_buy = safe_get(row, "融资买入额(元)", "融资买入额") / 1e8
                margin_repay = safe_get(row, "融资偿还额(元)", "融资偿还额") / 1e8
                margin_balance = safe_get(row, "融资余额(元)", "融资余额") / 1e8
                short_sell = safe_get(row, "融券卖出量(股)", "融券卖出量") / 1e8
                short_balance = safe_get(row, "融券余额(元)", "融券余额(元)") / 1e8

                total = margin_balance + short_balance
                margin_ratio = (margin_balance / total * 100) if total > 0 else 0
                short_ratio = (short_balance / total * 100) if total > 0 else 0
                net_margin_flow = margin_buy - margin_repay

                sql = text("""INSERT INTO market_margin
                    (trade_date, total_margin_buy, total_margin_balance, total_short_sell, total_short_balance, data_source)
                    VALUES (:trade_date, :total_margin_buy, :total_margin_balance, :total_short_sell, :total_short_balance, :data_source)
                    ON DUPLICATE KEY UPDATE
                    total_margin_buy = VALUES(total_margin_buy), total_margin_balance = VALUES(total_margin_balance),
                    total_short_sell = VALUES(total_short_sell), total_short_balance = VALUES(total_short_balance)""")
                await session.execute(sql, {
                    "trade_date": str(today),
                    "total_margin_buy": round(margin_buy, 4),
                    "total_margin_balance": round(margin_balance, 4),
                    "total_short_sell": round(short_sell, 4),
                    "total_short_balance": round(short_balance, 4),
                    "data_source": "akshare",
                })
            await session.commit()

        logger.info("[Scheduler] margin update done")

    except Exception as e:
        logger.error("[Scheduler] margin update failed: %s", e)


# ============================================================
# 任务 13：指数情绪映射（market_sentiment -> index_sentiment）
# ============================================================
...
async def _run_index_sentiment_update() -> None:
    """
    每日指数情绪映射：从 market_sentiment 读取当日快照，映射到 index_sentiment（V4兼容表）。
    在市场快照后执行（15:50）。
    """
    if not await _is_trade_day():
        logger.info("[Scheduler] 今日无交易数据，跳过指数情绪映射")
        return
    ...
    today = date.today()
    logger.info("[Scheduler] 开始每日指数情绪映射 -- %s", today.isoformat())

    engine = get_async_engine()
    inserted = 0

    async with AsyncSession(engine) as session:
        for index_code, index_name in INDEX_SENTIMENT_MAP.items():
            try:
                ...
                if ms is None:
                    logger.warning("[Scheduler] %s no market_sentiment data", index_code)
                    continue

                score = float(ms.composite_score) if ms.composite_score else 50.0
                label = ms.sentiment_label or "中性"

                td_str = today.isoformat()  # factor_history uses YYYY-MM-DD format
                # Get current CLOSE for price
                r1 = await session.execute(text(
                    "SELECT raw_value FROM factor_history WHERE index_code=:code AND factor_name='CLOSE' AND trade_date=:td"
                ), {"code": index_code, "td": td_str})
                price_row = r1.first()
                price = float(price_row[0]) if price_row else None

                # Calculate pct_change from current vs previous CLOSE
                # Look back up to 7 days to find the previous trading day
                prev_close = None
                for lookback in range(1, 8):
                    prev_date_str = (today - timedelta(days=lookback)).isoformat()
                    r_prev = await session.execute(text(
                        "SELECT raw_value FROM factor_history WHERE index_code=:code AND factor_name='CLOSE' AND trade_date=:td"
                    ), {"code": index_code, "td": prev_date_str})
                    prev_row = r_prev.first()
                    if prev_row:
                        prev_close = float(prev_row[0])
                        break
                pct_change = round((price - prev_close) / prev_close * 100, 4) if (price and prev_close) else None

                if score >= 70: short_trend = "上涨"
                elif score >= 45: short_trend = "震荡"
                else: short_trend = "下跌"

                sql = text("""INSERT INTO index_sentiment
                    (calc_date, index_code, index_name, composite_score, sentiment_label,
                     short_trend, mid_trend, long_trend, price, pct_change)
                    VALUES (:calc_date, :index_code, :index_name, :composite_score, :sentiment_label,
                     :short_trend, :mid_trend, :long_trend, :price, :pct_change)
                    ON DUPLICATE KEY UPDATE
                    composite_score = VALUES(composite_score), sentiment_label = VALUES(sentiment_label),
                    short_trend = VALUES(short_trend), price = VALUES(price), pct_change = VALUES(pct_change)""")
                await session.execute(sql, {
                    "calc_date": str(today), "index_code": index_code, "index_name": index_name,
                    "composite_score": round(score, 2), "sentiment_label": label,
                    "short_trend": short_trend, "mid_trend": short_trend, "long_trend": short_trend,
                    "price": price, "pct_change": pct_change,
                })
                inserted += 1

            except Exception as e:
                logger.warning("[Scheduler] %s sentiment mapping failed: %s", index_code, e)
...
def init_scheduler() -> AsyncIOScheduler:
    ...
    # 任务 11：每个交易日 15:35 执行板块价格更新（AKShare）
    scheduler.add_job(
        _run_sector_price_update,
        trigger=CronTrigger(
            day_of_week="mon-fri",
            hour=15,
            minute=35,
        ...
        id="daily_sector_price_update",
        name="每日板块价格更新",
        replace_existing=True,
    )

    # 任务 12：每个交易日 16:15 执行融资融券更新（AKShare）
    scheduler.add_job(
        _run_market_margin_update,
        trigger=CronTrigger(
            day_of_week="mon-fri",
            hour=16,
            minute=15,
        ...
        id="daily_market_margin_update",
        name="每日融资融券更新",
        replace_existing=True,
    )

    # 任务 13：每个交易日 15:50 执行指数情绪映射（market_sentiment -> index_sentiment）
    scheduler.add_job(
        _run_index_sentiment_update,
        trigger=CronTrigger(
            day_of_week="mon-fri",
            hour=15,
            minute=50,
        ...
        id="daily_index_sentiment_update",
        name="每日指数情绪映射",
        replace_existing=True,
    )
