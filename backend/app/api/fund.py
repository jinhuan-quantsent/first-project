...
async def get_fund_sectors(
    ...
    # 获取概念板块列表（默认返回热门板块，与基金间接相关）
    # 若需要「基金持仓对应板块」，需先查 top_holdings 再做映射，当前先用通用板块行情
    ...
    if items:
        ...
        return {
            ...
            "data": {
                "items": scored[:20],
                "total": raw.get("total", len(scored)) if isinstance(raw, dict) else len(scored),
                "fund_code": fund_code,
            },
            "message": "ok",
        }
    ...
    return {
        "code": 0,
        "data": {"items": [], "total": 0, "fund_code": fund_code},
        "message": "暂无板块数据",
    }
