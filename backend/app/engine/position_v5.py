...
class PositionEngineV5:
    ...
    async def calculate(
        ...
        return {
            ...
            "reason": reason,
            "trend_text": trend_guard_data.get("trend_narrative", ""),
...
async def calculate_trend_guard(
    ...
    """
    计算趋势卫士完整数据（MA20+MACD双指标趋势判定）

    调用 trend_guard.calculate_position_v5 获取趋势卫士完整结构化数据。
    返回包含趋势信号、MACD信号、闸门状态、操作建议等的字典。
    """
    # 检查配置开关
    ...
    try:
        ...
        return {
            ...
            "gate_triggered": result.get("gate_triggered"),
    ...
    try:
        ...
        return result.get("trend_narrative", "")
    except Exception as e:
        logger.error(f"趋势卫士调用失败: {e}")
        return ""
